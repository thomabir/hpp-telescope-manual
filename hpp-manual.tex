...
\begin{titlingpage}
    \begin{center}
        ...
        \vspace{3cm}
        {\scalebox{1.8}{\Huge\bfseries \addfontfeature{LetterSpace=2.0}
                HPP TELESCOPE}}\\ \vspace{0.6cm} {\LARGE\bfseries \addfontfeature{LetterSpace=2.0} USER MANUAL
            $
                \bullet
            $
            EXPERIMENTS
...
\noindent Current version of this manual \& LaTex source code: \url{github.com/XXXXXXX}
...
\software{Python} comes pre-installed on Linux and MacOS, while on Windows it is not prepackaged.

Windows users can download and install \software{Python} from its website \url{https://www.python.org/downloads/windows/}.
Install \software{Python} 3 with one of the 3.6.
* versions.
You can find an example of a step by step guide at \url{https://www.howtogeek.com/197947/how-to-install-python-on-windows/}.

The next step is to setup a \software{Python} environment.
...
\end{minted}
By typing 'which \software{Python}', the path to the bin folder into the virtual environment should be displayed, e.g., 'venv/bin/\software{Python}'.
This has to be done again every time you open a new terminal window or tab.

Every time you have to install a new \software{Python} package, you simply type
\begin{minted}{bash}
    pip install <package\_name>
\end{minted}

For \textbf{Windows} users: if you install \software{Python}2 >2.7.9 or \software{Python} 3 >3.4 from \url{https://www.python.org/downloads/windows/}, then 'pip' should be already installed.
...
\subsubsection{List of \software{Python}
    Packages}

The following is the list of \software{Python} packages you need for the data reduction:
\begin{itemize}
    \item numpy
    \item \software{astropy}
    \item photutils
    \item astroscrappy
    \item astroalign
