...
Each science frame has to be corrected with a master dark frame and a master flat frame, following
\begin{equation}
    \label{eq:correction}
    \text{calibrated science frame} = \frac{\text{raw science frame} - \text{master dark} }{\text{normalised master flat}}.
\end{equation}
Note that the master dark frame has to have the same exposure time as the science frame that you are correcting, and that the master flat has to have the same filter as the science frame.

The master dark frame for a specific exposure time is the pixel-wise median of all dark frames with this exposure time:
\begin{minted}{python}
    master_dark_5s = np.median(dark_frames_5s, axis=0)
\end{minted}
This assumes that you have made a list that contains all the dark frames with \SI{5}{\second} exposure time.
The purpose of the median is to reduce random noise, and to reject outliers, such as cosmic rays.

Similarly, the master flat frame is the pixel-wise median of the normalised flat frames:
\begin{minted}{python}
    normalised_flats_R = [flat / np.median(flat) for flat in flats_R]
    master_flat_R = np.median(normalised_flats_R, axis=0)
\end{minted}
We normalise them first because the brightness of the twilight sky changes quickly from frame to frame, and we want to compensate for that.

The calibrated science frame is then calculated according to \cref{eq:correction}:
\begin{minted}{python}
    calibrated_science_001 = (science_001 - master_dark) / master_flat
\end{minted}
